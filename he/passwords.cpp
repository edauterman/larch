...
SEALContext SetContext() {
    ...
    size_t poly_modululs_degree = 4096;
...
PwdClient(SEALContext &context_) {
    context = context_;
}
...
uint128_t PwdClient::Decrypt(Ciphertext &c) {
...
PwdServer(SealContext &context_, PublicKey &public_key_) {
    context = context_;
    public_key = public_key_;
}

Ciphertext PwdServer::Eval(Ciphertext *c, uint128_t *inputs) {
    ...
    for (int i = 0; i < num_pwds; i++) {
        Plaintext plain(uint64_to_hex_string(input[i]));
        Ciphertext res;

...
impl ProofSystem for ZkInterface {
    ...
    fn generate_proof(
        ...
        let mut out_file1 = File::create(proof_path).unwrap();
        let mut out_file2 = File::create(pk_path).unwrap();
        generate_proof(program, witness, &mut out_file1, &mut out_file2)
    }
...
fn write_r1cs<W: Write>(
    ...
    let args = ConstraintSystemArgs { constraints: Some(vector_offset), info: None };

    let r1cs_constraints = ConstraintSystem::create(&mut builder, &args);
    //let r1cs_constraints = R1CSConstraints::create(&mut builder, &args);
    let root_args = RootArgs { message_type: Message::ConstraintSystem, message: Some(r1cs_constraints.as_union_value()) };
...
fn write_circuit<W: Write>(
    ...
    let ids_vec = builder.create_vector(&(0..first_private_id).collect::<Vec<u64>>()[..]);
    let values_vec = builder.create_vector(&values.unwrap()[..]);
    let vars = Variables::create(&mut builder, &VariablesArgs {
        variable_ids: Some(ids_vec),
        //variable_ids: Some((0..first_private_id).collect().as_union_value()),
        values: Some(values_vec),
        info: None,
    });

    let field_vec = builder.create_vector(&convert_field(&FieldPrime::max_value())[..]);
    let header_args = CircuitHeaderArgs {
        instance_variables: Some(vars),
        free_variable_id: free_variable_id,
        field_maximum: Some(field_vec),
        configuration: None,
    };
    let assign = CircuitHeader::create(&mut builder, &header_args);
    let message = Root::create(&mut builder, &RootArgs {

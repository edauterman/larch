...
void AssembleShares(uint32_t *in0, uint32_t *in1, uint32_t *in2, uint8_t *out, int num_blocks) {
    bool *bs = (bool *)malloc(num_blocks * sizeof(bool));
...
void VerifyDeserializeCtCircuit(uint8_t *proof_bytes, int numRands, __m128i iv, int m_len, int in_len, uint8_t * hashOutRaw, uint8_t *keyCommRaw, uint8_t *ctRaw, bool *ret) {
    Proof p;

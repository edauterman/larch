...
void check_ciphertext_circuit(CircuitExecution *ex, block hash_out[], block m[], int m_len, block hash_in[], int in_len, block ct[], const __m128i iv, block key[], block key_comm[], block key_r[], block res[]) {
    *res = ex->public_label(1);
    ...
    for (int i = 0; i < m_len; i++) {
        block out = ex->xor_gate(m[i], hash_in[i]);
        out = ex->not_gate(out);
        *res = ex->and_gate(*res, out);
    }
    //printf("num and = %d\n", ex->num_and());

    // Check hash matches
    for (int i = 0; i < 256; i++) {
        block out = ex->xor_gate(hash_out[i], hash_out_calc[i]);
        out = ex->not_gate(out);
        *res = ex->and_gate(*res, out);
    }
    ...
    // Check hash matches
    for (int i = 0; i < 256; i++) {
        block out = ex->xor_gate(key_comm[i], hash_out_calc[i]);
        out = ex->not_gate(out);
        *res = ex->and_gate(*res, out);
    }
    ...
    AES_128_CTR_Calculator aes128_calc = AES_128_CTR_Calculator(ex);
    aes128_calc.aes_128_ctr(key, iv, m, ct_calc, m_len, 0);

    // Check ciphertext matches
    for (int i = 0; i < m_len; i++) {
        block out = ex->xor_gate(ct_calc[i], ct[i]);
        out = ex->not_gate(out);
        *res = ex->and_gate(*res, out);
    }
